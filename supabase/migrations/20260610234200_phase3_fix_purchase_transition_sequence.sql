...
declare
  ...
  v_next_status app.purchase_request_status;
begin
  ...
  -- Enforce legal transition sequence:
  -- created -> ordered -> partially_received|received
  if v_pr.status = 'created' then
    perform app.require_legal_transition('purchase_requests', 'created', 'ordered');
  ...
  v_next_status := (
    case
      when v_pr.quantity_received + p_received_quantity < v_pr.quantity_requested then 'partially_received'
      else 'received'
    end
  )::app.purchase_request_status;

  perform app.require_legal_transition('purchase_requests', v_pr.status::text, v_next_status::text);
